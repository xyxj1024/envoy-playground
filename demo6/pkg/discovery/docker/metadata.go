...
/* GetCurrentContainerID attempts to extract the current container ID
 * from the provided file paths. If no files paths are provided, it will
 * default to /proc/1/cpuset, /proc/self/cgroup and /proc/self/mountinfo.
 * It attempts to match the HOSTNAME first then use the fallback method,
 * and returns with the first valid match.
 */
 ...
 * KEY, KEY=, KEY=VALUE  or KEY=NESTED_KEY=VALUE2, and
 * returns a map[string]string where items are split at their first `=`.
 */
